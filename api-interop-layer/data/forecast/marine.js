...
export const getMarineDays = async (hours, timezone) => {
  ...
  const firstDay = await new Promise((resolve) =>
    resolve(dayjs.utc(Array.from(hours.keys())[0]).tz(timezone)),
  )
    .then(async (day) => {
      if (day.get("hour") < 6) {
  ...
  const lastDay = await new Promise((resolve) =>
    resolve(dayjs.utc(Array.from(hours.keys()).pop()).tz(timezone)),
  )
    .then((day) => {
  ...
  return [...Array(dayjs.duration(lastDay.diff(firstDay)).asDays())].map(
    (_, day) => {
