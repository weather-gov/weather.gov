...
describe("Observation fetch caching tests", () => {
  ...
  describe("in the happy path", () => {
    ...
    beforeEach(() => {
      ...
      stationsResponse.body.json.resolves(stationsResponseBody);
      ...
      singleStationResponse.body.json.resolves(responseBody);
    });
    ...
    it("caches the stations list (with the default TTL regardless of the headers)", async() => {
      ...
      expect(connectionPool.request.calledWith(
    ...
    it("returns a cached value for the stations list, if one is present", async () => {
      ...
      connectionPool.request
        .withArgs(sinon.match({path: `/stations/cachedStation1/observations?limit=1`}))
        .resolves(singleStationResponse);

      // The cached endpoint that we are testing
      getFromRedis.withArgs(`/gridpoints/TST/2,3/stations?limit=3`).resolves(cachedValue);
      await getObservations(
        {
          grid: { wfo: "TST", x: 2, y: 3},
          point: { latitude: 22.2, longitude: 33.3 },
      ...
      expect(getFromRedis.calledWith(`/gridpoints/TST/2,3/stations?limit=3`)).to.equal(true);
      ...
      // arrray
      expect(connectionPool.request.calledWith(
        sinon.match({path: `/stations/cachedStation1/observations?limit=1`})
      )).to.equal(true);
      expect(connectionPool.request.calledWith(
        sinon.match({path: `/stations/cachedStation2/observations?limit=1`})
      )).to.equal(true);
    });
    ...
    it("caches the individual station data", async () => {
      ...
      connectionPool.request
        .withArgs(sinon.match({path: `/stations/station1/observations?limit=1`}))
        .resolves(singleStationResponse);
    ...
    it("returns cached data for an individual station that is cached", async () => {
      const cachedData = Object.assign(
        {},
        responseBody.features[0].properties
      );
      cachedData.temperature.value = 22;
      
      getFromRedis.withArgs(`/stations/station1/observations?limit=1`).resolves(cachedData);
