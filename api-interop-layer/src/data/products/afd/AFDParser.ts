...
export default class AFDParser {
  ...
  parse() {
    ...
    const sections = AFDParser.splitIntoTopicSections(body);
  ...
  parseDocumentPreamble() {
    ...
    for (let i = 0; i < preambleParagraphs.length; i++) {
      ...
      const text = AFDParser.normalizeSpaces(paragraph).trim();
      if (text !== "") {
  ...
  parseHeader(str: any) {
    ...
    if (genericMatch) {
      ...
      return str.slice(genericMatch.index + genericMatch[0].length);
    }

    return str;
  }

  parseSubheader(str: any) {
    ...
    if (match) {
      ...
      return match.input.slice(match.index + match[0].length).trim();
    }

    return str;
  }

  parseTextContent(str: any) {
    if (str === "") {
      return str;
    }
    switch (this.contentType) {
      ...
      default:
        return this.parsedNodes.push({
          type: "text",
          content: AFDParser.normalizeSpaces(
            str.trim().replaceAll("\n", " "),
          ),
        });
    }
  ...
  parseTempsTableContent(str: any) {
  ...
  getStructureForTwig() {
    const preambleCode: any[] = [];
    const preambleText: any[] = [];
    const body: any[] = [];
    const epilogue: any[] = [];
   ...
   * into a single space
   */
  static normalizeSpaces(str: any) {
   ...
   */
  static splitIntoTopicSections(str: any) {
