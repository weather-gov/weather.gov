...
export const getClosestPlace = async (latitude: any, longitude: any) => {
...
export const getPointData = async (latitude: any, longitude: any) => {
  pointLogger.trace({ latitude, longitude }, "place");
  const point = { latitude, longitude };

  const pointsPromise = fetchAPIJson(`/points/${latitude},${longitude}`).then((data) => {
      if (data.error) {
        return data;
      }
