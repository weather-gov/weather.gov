 ...
 */
export const modifyTimestampsForAlert = (alert: any, timezone: any = "UTC") => {
 ...
 */
export const alignAlertsToDaily = (alerts: any, days: any) => {
  for (const day of days.filter(({ hours }) => hours.length > 0)) {
    ...
    // particular day.
    const dayAlerts = alerts.items.filter(({ onset, finish }) =>
