...
describe("AlertsCache tests", () => {
  ...
  beforeEach(() => {
    ...
    alertsCache.db = (global as any).test.database;

    // Throw if there are any un-mocked database calls. We want to catch
    // those because they aren't being controlled for or tested.
    (global as any).test.database.query.rejects(new Error("Unexpected query"));
  });

  it("#getHashes", async () => {
    const query = `SELECT hash FROM ${alertsCache.tableName}`;
    (global as any).test.database.query.withArgs(sinon.match(query)).resolves({
  ...
  describe("#removeByHashes", () => {
    ...
    it("with just one hash", async () => {
      ...
      (global as any).test.database.query.withArgs(query, toRemove).resolves(true);
  ...
  describe("#add with land alert kind", () => {
    it("has GeoJSON shape", async () => {
      ...
      (global as any).test.database.query
        .withArgs(sinon.match(new RegExp(`^${setGeometryQuery}$`, "m") as any), [hash])
        .resolves();
  ...
  it("#getIntersectingAlertsForPoint", async () => {
    ...
    (global as any).test.database.query.withArgs(query).resolves({ rows: output });
