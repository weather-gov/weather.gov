...
export class AlertsCache {
   ...
   */
  determineOldHashesFrom(currentHashes: any, incomingHashes: any) {
   ...
   * See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set/difference
   */
  determineNewHashesFrom(currentHashes: any, incomingHashes: any) {
   ...
   */
  async removeByHashes(hashes: any) {
    if (!Array.isArray(hashes) || hashes.length === 0) {
      return [];
    }

    const list = hashes.map((_, i: any) => `$${i + 1}`).join(",");
   ...
   */
  async add({
    hash, alert, geometry = null, counties = [], states = [], alertKind = null, }: { hash: any; alert: any; geometry?: any; counties?: any[]; states?: any[]; alertKind?: any }) {
   ...
   */
  async getIntersectingAlertsForPoint(lat: any, lon: any, options: any) {
    const inputGeometry = [
  ...
  async getAlertsForCountyFIPS(fips: any) {
