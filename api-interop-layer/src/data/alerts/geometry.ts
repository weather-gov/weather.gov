...
const unwindGeometryCollection = (geojson: any, parentIsCollection = false) => {
...
export const generateAlertGeometry = async (db: any, rawAlert: any, { parent = parentPort }: any = {}) => {
  ...
  const zones = (rawAlert.properties.affectedZones ?? []).filter((zoneId) => !/\/county\//.test(zoneId),
  );
