...
export const updateAlerts = async ({
  ...
  const rawAlerts = await fetchAPIJson("/alerts/active?status=actual").then(({ error, features }) => {
    if (error) {
      return { error: true };
    }
  ...
  for await (const rawAlert of alertsToUpdate) {
    ...
    const geometry = await (generator || generateAlertGeometry)(db, rawAlert);
