...
describe("Forecast index", () => {
  ...
  describe("for a land point", () => {
    ...
    it("calls the expected endpoints", async () => {
      ...
      expect((fetch as any).calledWith(url`/gridpoints/TST/49488,34`)).to.be.true;
      expect((fetch as any).calledWith(url`/gridpoints/TST/49488,34/forecast`)).to.be
        .true;
      expect((fetch as any).calledWith(url`/gridpoints/TST/49488,34/forecast/hourly`)).to
        .be.true;
    });
  });
