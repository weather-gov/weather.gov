...
func GetForecast(grid *Grid, place *Place, isMarine bool) (*ForecastResult, error) {
	...
	go func() {
		...
		// Better: FetchAPIJson should generic or we assume map and traverse manually?
		// Since we have complex structs (`GridpointResponse`, `ForecastDailyResponse`),
		// it's easier to use JSON marshalling roundtrip OR change FetchAPIJsond to return []byte option?
		// But FetchAPIJson handles caching and errors returning maps.
	...
	// convertValue logic?
	// `convert.go` usually exports ConvertProperties. `convertValue` might be internal or separate export.
	...
	// ConvertProperties handles object traversal.
