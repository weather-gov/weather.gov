...
const STANDARD_HEADERS: Record<string, string> = process.env.API_KEY
...
const internalFetch = async (path: string) => {
  ...
  return fetch(url.toString(), { headers }).then(async (r) => {
    ...
    if (r.status >= 200 && r.status < 400) {
      ...
      if (USE_REDIS && !isGHWO && !isAlert) {
        // Validation: getTTLFromResponse expects Response. r is Response (global fetch).
        // However, TS checks might complain if global Response is not perfectly aligned with what getTTLFromResponse expects.
        // getTTLFromResponse uses r.headers.get("Cache-Control").
        // We will cast to any or ensure type compatibility.
        const ttl = getTTLFromResponse(r as unknown as Response);
        if (ttl) {
...
export const fetchAPIJson = async (path: string, { wait = sleep } = {}) =>
  internalFetch(path)
