...
const setupMap = () => {
    ...
    const filterMap = () => {
      if (curDayIndex !== "all") {
        countyAlertLayers["all"].forEach(layer => layer.setStyle({
            fillOpacity: 0,
            opacity: 0
        }));
      }
  ...
  const ESRI_API_KEY = "AAPK1dd93729edc54e84ade1ea5dc0f4f9d3EPexfd5qirlO3QtHGBj5JQL7iUYHQOb4yLjfKEYFLcyN9PlMd87lMjjv8D3DxDsQ";
  L.esri.Vector.vectorBasemapLayer("arcgis/streets", { apiKey: ESRI_API_KEY }).addTo(map);
  ...
  const selected = document.querySelector("wx-tabs button[aria-selected='true']");
      ...
      if (result.state === 'granted') {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            const locationMarker = document.querySelector(".leaflet-marker-icon.wx-location-marker");
