...
export default class Listbox extends HTMLElement {
  ...
  bindEventListeners() {
    ...
    this.shadowRoot
      .querySelector("slot")
      .addEventListener("slotchange", this.handleItemsChanged);
  ...
  handleItemsChanged(event) {
    ...
    Array.from(host.querySelectorAll(`[role="option"]`)).forEach(
      (option, idx) => {
  ...
  pseudoFocusItem(element) {
    ...
    if (element) {
      ...
      if (element.offsetTop < this.scrollTop) {
        ...
        this.scrollTop =
          element.offsetTop - this.offsetHeight + element.offsetHeight;
      }
  ...
  selectItem(element, fireEvent = true) {
    ...
    Array.from(this.querySelectorAll(`[aria-selected="true"]`)).forEach(
      (selectedEl) => {
  ...
  static get observedAttributes() {
    return ["selection-follows-focus"];
  }
}
