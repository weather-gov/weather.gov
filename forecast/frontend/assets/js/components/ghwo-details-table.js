class GHWODetailsTable extends HTMLElement {
  ...
  connectedCallback(){
    ...
    Array.from(this.querySelectorAll(".wx-scroll-button")).forEach(scrollButton => {
  ...
  handleChicletClick(event){
    ...
    Array.from(table.querySelectorAll(`.ghwo-chiclet[role="button"]`)).forEach(button => {
  ...
  scrollClickRight(wrapper){
    const containerWidth = wrapper.getBoundingClientRect().width;
    const firstColumnWidth = wrapper.querySelector(
    ...
    if(nextCol){
      ...
      if(Math.floor(nextLeftPos) <= Math.floor(wrapper.scrollLeft)){
        nextLeftPos = nextCol.nextElementSibling.offsetLeft - firstColumnWidth - 2;
      }
  ...
  arrowUp(event){
    ...
    if(!isFirstRow){
      const prevCell = currentRow.previousElementSibling.querySelector(`td:nth-child(${elementIndex + 1})`);
  ...
  arrowDown(event){
    ...
    const elementIndex = Array.from(currentRow.children).findIndex(el => el === currentCell);
    if(!isLastRow){
      const nextCell = currentRow.nextElementSibling.querySelector(`td:nth-child(${elementIndex + 1})`);
  ...
  pageDown(event){
    ...
    const elementIndex = Array.from(currentRow.children).findIndex(el => el === currentCell);
    if(!isLastRow){
      const nextCell = currentRow.parentElement.querySelector(`tr:last-of-type > td:nth-of-type(${elementIndex + 1})`);
      const nextButton = nextCell.querySelector(".ghwo-chiclet");
      nextButton.focus();
      event.preventDefault();
    }
  ...
  pageUp(event){
    ...
    if(!isFirstRow){
      const prevCell = currentRow.parentElement.querySelector(`tr:first-of-type > td:nth-of-type(${elementIndex + 1})`);
      const prevButton = prevCell.querySelector(".ghwo-chiclet");
      prevButton.focus();
      event.preventDefault();
    }
  ...
  home(event){
    ...
    if(!isFirstElement){
      let firstCell = currentRow.querySelector("td:first-of-type  .ghwo-chiclet");
      if(event.ctrlKey){
        // If control is also pressed, we navigate to the first cell
        // in the first row
        const firstRow = currentRow.parentElement.querySelector(`tr:first-of-type`);
    ...
    if(!isLastElement){
      ...
      if(event.ctrlKey){
        ...
        const lastRow = currentRow.parentElement.querySelector(`tr:last-of-type`);
