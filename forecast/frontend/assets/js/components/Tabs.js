...
class Tabs extends HTMLElement {
  ...
  connectedCallback() {
    Array.from(this.querySelectorAll('[role="tab"]')).forEach((tabElement) => {
   ...
   */
  handleKeyDown(event) {
    const currentFocus = this.querySelector(
      '[role="tab"]:focus,[role="tab"]:focus-within',
    );
    const firstItemInFocus = currentFocus.matches(
      '[role="tab"]:first-child:focus, [role="tab"]:first-child:focus-within',
    );
    const lastItemInFocus = currentFocus.matches(
      '[role="tab"]:last-child:focus, [role="tab"]:last-child:focus-within',
    );
    let handled = false;
