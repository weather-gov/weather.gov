...
import {
  getRecentLocations,
  addRecentLocation,
} from "../../assets/js/components/recent-locations.js";

describe("Recent locations tests", () => {
  describe("getRecentLocations", () => {
    ...
    it("returns the recent locations from localStorage", () => {
      const expected = [
      ...
      const actual = getRecentLocations();
  ...
  describe("addRecentLocation", () => {
    ...
    it("saves the recent location to localStorage when there are no other saved items", () => {
      const example = {
        text: "Hobbiton, The Shire",
        url: "/middle-earth/shire/hobbiton/",
      };
      const expected = JSON.stringify([example]);

      addRecentLocation(example);
    ...
    it("prepends the recent location to exiting list in localStorage, when there are already saved items", () => {
      const existingItems = [
        { text: "Hobbiton, The Shire", url: "/middle-earth/shire/hobbiton/" },
      ];
      ...
      const newItem = {
        text: "Edoras, Rohan",
        url: "/middle-earth/rohan/edoras/",
      };
      const expected = JSON.stringify([newItem, ...existingItems]);

      addRecentLocation(newItem);
