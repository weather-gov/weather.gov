...
if (missing.length) {
  ...
  missing.forEach((key) => {
    ...
    if (entryList) {
      ...
      entryList.map((entry) =>
        console.error(`\t\t${entry.filename}:${entry.lineNumber}`),
      );
    }
  });
} else {
  console.log(
    `${GREEN_SUCCESS} No English translations are missing from en.po.`,
  );
  console.log(
    "Caution: This test won't help if you forget to mark a string for translation.",
  );
}

const ignored = new Set(config.suppress.stale.en ?? []);
const stale = translationKeys
  .filter((key) => !translatable.includes(key))
...
if (stale.length) {
  ...
  console.log(
    `${GREEN_SUCCESS} All English translations in en.po are used in Twig or PHP`,
  );
}
...
languages.forEach((langCode) => {
  ...
  const missing = translationKeys
    .filter((key) => !comparisonKeys.includes(key) || !keyLookup[key]?.msgstr)
    .filter((key) => !ignore.has(key));
  if (missing.length) {
    hasErrors = true;
    console.error(
  ...
  if (stale.length) {
    ...
    console.log(
      `${GREEN_SUCCESS} All strings from ${langCode}.po are in en.po.`,
    );
  }
});
