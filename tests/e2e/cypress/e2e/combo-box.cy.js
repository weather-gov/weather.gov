...
describe("wx-combo-box tests", () => {
  ...
  describe("When typing into the input", () => {
    beforeEach(() => {
      ...
      // Go to landing page
      cy.visit("http://localhost:8080/");
      cy.get("wx-combo-box > input").focus().type("Arlin");
      cy.wait("@arcSearch");
    });
    ...
    it("pressing Enter selects the second item, but does not submit it", () => {
      cy.get("wx-combo-box input").type("{downArrow}{downArrow}{enter}");
      cy.wait("@arcSearchItems");

      cy.location('pathname').should("eq", "/");
    });
    it.skip("clicking selects the second item and navigates to the location page", () => {
      cy.intercept("/point/*").as("pointNavigation");
      cy.wait(200);
      cy.get("wx-combo-box ul").find("li:nth-child(3)").trigger("click");
      cy.wait("@arcSearchItems");
    ...
    it.skip("pressing enter twice performs the navigation on an item", () => {
      cy.intercept("/point/*").as("pointNavigation");
