...
describe("WFO PDF upload tests", () => {
  test("uploader can upload wfo pdfs", async ({ request }) => {
    ...
    const firstResponse = await uploadFile(
      request,
      uploadLocation,
      pdfFilename,
    );
    ...
    const secondResponse = await uploadContentType(
      request,
      contentLocation,
      data,
    );
  ...
  test("uploader cannot upload wfo pdfs without wfo code", async ({
    request,
  }) => {
    // step one: upload the PDF
    const pdfFilename = "test-upload.pdf";
...
describe("weather story upload tests", () => {
  ...
  test("uploader cannot upload wfo weather story images with missing attributes", async ({
    request,
  }) => {
    // step one: upload the image
    const pngFilename = "test-upload.png";
  ...
  const uploadWeatherStoryWithAttributes = async (request, attributes) => {
    ...
    const firstFullResponse = await uploadFile(
      request,
      uploadFullLocation,
      pngFilename,
    );
    ...
    const firstSmallResponse = await uploadFile(
      request,
      uploadSmallLocation,
      pngFilename,
    );
  ...
  test("uploader can upload wfo weather story images with all attributes", async ({
    request,
  }) => {
  ...
  test("uploader cannot upload wfo weather story images without wfo", async ({
    request,
  }) => {
    const response = await uploadWeatherStoryWithAttributes(request, {});
    expect(response.status()).toEqual(422);
