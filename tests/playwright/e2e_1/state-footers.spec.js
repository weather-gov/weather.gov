...
describe("State page county links", () => {
  ["alerts", "risks", "radar", "analysis"].forEach((tabName) => {
    test(`Displays county links url on ${tabName} state tab`, async ({
      page,
    }) => {
    ...
    test(`County links on ${tabName} tab takes us to correct page and scroll position`, async ({
      page,
    }) => {
      const response = await page.goto(
        services.webApp(`/forecast/state/NY/${tabName}`),
      ...
      await expect(await page.getByText("Albany, NY")).toBeInViewport();
    });
  });
});
...
describe("State WFO footer links", () => {
  ["alerts", "risks", "radar", "analysis"].forEach((tabName) => {
    test(`Displays the WFO links section on the ${tabName} tab`, async ({
      page,
    }) => {
      const response = await page.goto(
        services.webApp(`/forecast/state/NY/${tabName}`),
      ...
      const header = await page.getByRole("heading", {
        name: "Weather forecasting offices",
      });
