...
const PRODUCT_TYPE_LOCATION_RX = /\/products\/types\/[A-Za-z]+\/locations\/([A-Za-z]+)\.json/;
...
const getProductFilePaths = (base, bundleName) => {
  return globSync(
    path.join(base, bundleName, "products", "**", "*.json")
  );
};

const getProductTypeFromPath = (path) => {
  const match = path.match(PRODUCT_TYPE_IN_PATH_RX);
  if(!match){
    return null;
  }
...
const getProductTypeFromData = (data) => {
  return data.productCode || "UNKNOWN";
};

const getWFOCodeFromPath = (path) => {
  const match = path.match(PRODUCT_TYPE_LOCATION_RX);
  if(!match){
    return null;
  }
...
const getProductLabel = (data, filePath, recordType) => {
  ...
  const productType = getProductTypeFromPath(filePath) ?? getProductTypeFromData(data);
...
const getProductInfo = async (base, bundleName) => {
  ...
  return await Promise.all(
    productFiles.map(getProductInfoForFilePath)
  );
};
